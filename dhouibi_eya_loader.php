<?php

class DhouibiEyaLoader extends MvcPluginLoader {

    var $db_version = '1.0';
    var $tables = array();

    function activate() {

        // This call needs to be made to activate this app within WP MVC

        $this->activate_app(__FILE__);

        // Perform any databases modifications related to plugin activation here, if necessary

        require_once ABSPATH.'wp-admin/includes/upgrade.php';

        add_option('dhouibi_eya_db_version', $this->db_version);

        // Use dbDelta() to create the tables for the app here
        // $sql = '';
        // dbDelta($sql);
        $sql = '
        CREATE TABLE '.$wpdb->prefix.'cvs (
          id int(11) NOT NULL auto_increment,
          nom varchar(255) NOT NULL,
          email varchar(255) default NULL,
          tel varchar(20) default NULL,
          titre varchar(255) default NULL,
          formations text default NULL,
          experiences text default NULL,
          domaine_id int(11) NOT NULL,
          PRIMARY KEY  (id)
        );
        CREATE TABLE '.$wpdb->prefix.'domaines (
          id int(11) NOT NULL auto_increment,
          domaine varchar(255) NOT NULL,
          active tinyint(1) NOT NULL,
          PRIMARY KEY  (id)
        )';
        dbDelta($sql);

    }

    function deactivate() {

        // This call needs to be made to deactivate this app within WP MVC

        $this->deactivate_app(__FILE__);

        // Perform any databases modifications related to plugin deactivation here, if necessary

    }

}
